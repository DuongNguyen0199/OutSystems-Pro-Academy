...
import { Lock, Eye, EyeOff, Key, Sparkles, Ticket, Gift } from 'lucide-react';
...
export default function CourseCard({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xs hover:shadow-md transition-shadow duration-200 flex flex-col">
      ...
      {/* Prometric Voucher Touchpoint */}
      <div className="px-5 md:px-6 py-2.5 bg-amber-50/60 border-b border-amber-100 flex flex-wrap items-center justify-between gap-2 text-[11px] font-medium text-amber-950">
        <span className="flex items-center gap-1.5">
          <Ticket className="w-3.5 h-3.5 text-amber-600 shrink-0" />
          <span>Registering on Prometric? Get an official 50-100% off voucher for this certification exam.</span>
        </span>
        <button
          onClick={onSelectVouchers}
          className="font-bold text-amber-700 hover:text-amber-900 hover:underline cursor-pointer flex items-center gap-1 shrink-0"
        >
