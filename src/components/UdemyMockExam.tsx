...
export default function UdemyMockExam({ course, onClose }: UdemyMockExamProps) {
  ...
  if (examPhase === 'intro') {
    return (
      <div className="fixed inset-0 w-screen h-screen bg-white z-50 flex flex-col font-sans select-none overflow-hidden">
        <div className="w-full h-full bg-white flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex overflow-hidden p-3 gap-3 bg-[#ffffff]">
            ...
            <div className="w-16 sm:w-20 flex flex-col gap-1.5 overflow-y-auto pr-1 py-1 select-none scrollbar-thin">
              {Array.from({ length: Math.min(24, questions.length) }).map((_, i) => (
                <button
                  ...
                  className={`w-[88%] h-8 rounded-r-md relative flex items-center justify-center font-bold text-xs cursor-pointer shrink-0 transition-all shadow-xs ${
                    i === 0 ? 'bg-[#76b82a] text-white font-extrabold' : 'bg-[#35454e] text-white hover:bg-[#435560]'
                  }`}
                >
                  {i !== 0 && (
                    <div 
                      className="absolute top-0 left-0 w-2.5 h-2.5 bg-[#a2b2bc]"
    ...
    return (
      <div className="fixed inset-0 w-screen h-screen bg-white z-50 flex flex-col font-sans select-none overflow-hidden">
        <div className="w-full h-full bg-white flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex overflow-hidden p-3 gap-3 bg-[#ffffff]">
            {/* Left Navigation Sidebar (Vertical Question Stack - Image 2 Match) */}
            <div className="w-16 sm:w-20 flex flex-col gap-1.5 overflow-y-auto pr-1 py-1 select-none scrollbar-thin">
              {questions.map((q, idx) => {
                ...
                if (isSelected) {
                  // Current Active Question: Lime-Green Arrow pointing Right (Image 2 #10)
                  return (
                    <button
                      key={q.id || idx}
                      onClick={() => setCurrentIndex(idx)}
                      className="w-full h-9 relative flex items-center justify-center font-black text-sm text-white bg-[#76b82a] hover:bg-[#68a424] cursor-pointer shrink-0 transition-all drop-shadow-md"
                      style={{
                        clipPath: 'polygon(0% 0%, 76% 0%, 100% 50%, 76% 100%, 0% 100%)'
                      }}
                      title={`Question ${qNum}`}
                    >
                      <span className="pr-2.5 font-mono">{qNum}</span>
                    </button>
                  );
                }
